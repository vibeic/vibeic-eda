...
```bash
docker pull ghcr.io/vibeic/vibeic-eda:0.2.66
```
...
docker run -d --init --name vibeic-eda ghcr.io/vibeic/vibeic-eda:0.2.66 --skip sleep infinity
...
docker run -d --init --name vibeic-eda \
  -v "$PWD:$PWD" -w "$PWD" \
  ghcr.io/vibeic/vibeic-eda:0.2.66 --skip sleep infinity
...
docker run -d --name vibeic-eda \
  ...
  ghcr.io/vibeic/vibeic-eda:0.2.66
...
docker run -it --rm -v "$PWD:/foss/designs/work" -w /foss/designs/work \
  ghcr.io/vibeic/vibeic-eda:0.2.66 bash
```
...
Current: **0.2.66** — the from-source release that is pinned by the plugin's
