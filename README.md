...
```bash
docker pull ghcr.io/vibeic/vibeic-eda:0.2.17
```
...
docker run -d --name vibeic-eda ghcr.io/vibeic/vibeic-eda:0.2.17 --skip sleep infinity
...
docker run -it --rm -v "$PWD:/foss/designs/work" -w /foss/designs/work \
  ghcr.io/vibeic/vibeic-eda:0.2.17 bash
```
...
Current: **0.2.17** — makes every EDA tool resolve on a **non-login `docker exec` PATH**
