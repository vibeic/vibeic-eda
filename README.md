...
```bash
docker pull ghcr.io/vibeic/vibeic-eda:0.2.25
```
...
docker run -d --name vibeic-eda ghcr.io/vibeic/vibeic-eda:0.2.25 --skip sleep infinity
...
docker run -d --name vibeic-eda \
  -v "$PWD:$PWD" -w "$PWD" \
  ghcr.io/vibeic/vibeic-eda:0.2.25 --skip sleep infinity
...
docker run -d --name vibeic-eda \
  ...
  ghcr.io/vibeic/vibeic-eda:0.2.25
...
docker run -it --rm -v "$PWD:/foss/designs/work" -w /foss/designs/work \
  ghcr.io/vibeic/vibeic-eda:0.2.25 bash
```
...
Current: **0.2.25** — adds the klayout `tl::Thread` `pthread_join` fix described above
